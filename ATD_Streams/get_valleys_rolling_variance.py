...
if __name__ == "__main__":
    ...
    output_dir = r"Y:\ATD\GIS\Valley Bottom Testing\Control Valleys\Rolling Variance Testing"
    ...
    output_folder = os.path.join(
        output_dir, 
        f"1000m_{rolling_window_size}_rollwin_{min_consecutive_decreases}_num_dec_{window_size}_win_{poly_order}_poly_{datetime.now().strftime('%Y-%m-%d_%Hh%Mm')}"
    )
    os.makedirs(output_folder, exist_ok=True)
    
    output_gpkg_name = f"Valley_Footprint_{rolling_window_size}_rollwin_{min_consecutive_decreases}_num_dec.gpkg"

...
def extract_max_depths_and_clip_rasters(shapefile, flow_raster_dict, segmented_polygons, output_dir):
    if isinstance(shapefile, str):
        points_gdf = gpd.read_file(shapefile)
    elif isinstance(shapefile, gpd.GeoDataFrame):
        points_gdf = shapefile
    ...
    for idx, point in enumerate(points_gdf.geometry):
        ...
        for flow_value, raster_path in flow_raster_dict.items():
            ...
            clip_output_path = os.path.join(output_dir, f"point_{idx}_flow_{flow_value}.tif")
...
if __name__ == '__main__':
    ...
    for prefix in prefix_list:
        ...
        try:
            points_gdf = ss.create_points_along_line(centerline_path, spacing = 5)
            ...
            stitched_raster_output_path = os.path.join(output_dir, 'stitched_output.tif')

            max_depths, raster_clips = extract_max_depths_and_clip_rasters(points_gdf, flow_raster_dict, segmented_polygons, output_dir_clips)
